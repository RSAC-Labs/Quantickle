...
const fileGraph = {
  ...
  layoutSettings: { currentLayout: 'timeline' }
};
...
window.GraphManager = {
  currentGraph: stripTimelineArtifacts(fileGraph),
  getCurrentGraphData() {
...
if (!hasTimelineBar) {
  throw new Error('Timeline bar missing after synchronizing sanitized file-based graph.');
}

const hasTimelineLink = Array.isArray(lastGraphData?.edges)
  && lastGraphData.edges.some(entry => (entry.data || entry).type === 'timeline-link');

if (!hasTimelineLink) {
  throw new Error('Timeline link missing after synchronizing sanitized file-based graph.');
}

const eventRecord = storedPositions.find(record => record.id === 'event-1');
if (!eventRecord || eventRecord.lockedX === undefined) {
  throw new Error('Locked X position was not preserved for sanitized file-loaded timeline node.');
}

if (connectorSyncs === 0) {
  throw new Error('Timeline connectors were not synchronized back into sanitized graph data for file-based graph.');
}

console.log('Sanitized file-loaded timeline graphs retain timeline scaffolding after timestamp edits.');
