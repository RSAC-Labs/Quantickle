...
global.fetch = async (filePath) => {
  if (filePath === '/assets/domains/index.json') {
    return { ok: true, json: async () => ({ files: ['assets/domains/hacktivist/ANONM0S.json'] }) };
  }
...
(async () => {
  ...
  assert.ok(window.NodeTypes['ANONM0S'], 'Hacktivist domain should be loaded');
  console.log('file-manager-hacktivist-autoload.test.js passed');
})();
