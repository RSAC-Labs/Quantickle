...
console.log('Tick positions and labels:', tickPositions, tickLabels);
...
const times = [Date.UTC(1995, 0, 1), Date.UTC(2021, 0, 1)];
...
const expectedA = ((Date.UTC(1995, 0, 1) - minTime) / range) * width;
...
if (Math.abs(nodeA.position('x') - expectedA) > 1 || Math.abs(nodeB.position('x') - expectedB) > 1) {
  throw new Error('Node positions do not align with numeric year timestamps');
}
...
console.log('Timeline layout handles numeric year timestamps');
process.exit(0);
