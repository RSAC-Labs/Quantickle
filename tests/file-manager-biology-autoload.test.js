...
global.fetch = async (filePath) => {
  if (filePath === '/assets/domains/index.json') {
    return { ok: true, json: async () => ({ files: ['assets/domains/biology/species.json'] }) };
  }
...
(async () => {
  ...
  assert.ok(window.NodeTypes.species, 'Biology domain should be loaded');
  console.log('file-manager-biology-autoload.test.js passed');
})();
